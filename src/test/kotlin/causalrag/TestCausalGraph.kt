...
class TestCausalGraph {
    ...
    /**
     * Verifies that loader accepts legacy `vertices` + index-based edge schema.
     */
    @Test
    fun testLoadLegacyVerticesSchema() {
        val legacyJson =
            """
            {
              "vertices": [
                { "hash_id": "entity-a", "content": "climate change" },
                { "hash_id": "entity-b", "content": "sea level rise" }
              ],
              "edges": [
        ...
        assertTrue(loaded)
        assertEquals(2, newBuilder.getGraph().numberOfNodes())
        assertEquals(1, newBuilder.getGraph().numberOfEdges())
        assertTrue(hasRelation("climate change", "sea level rise", newBuilder))
    }
