...
/**
 * Runs MusiQue evaluation using HippoRag for generation and ragas for scoring.
 */
object MusiQueHippoRagas {
    ...
    fun main(args: Array<String>) {
        ...
        val resultsDir = Path.of("eval_results").resolve("musique_hipporag_ragas_$timestamp")
                    ...
                    async(Dispatchers.IO) {
                        semaphore.withPermit {
                            try {
                                ...
                                val hippoRag = HippoRag(config)
                                hippoRag.index(documents)
                                val qaResult =
                                    when (semanticMode) {
                                        HippoRagSemanticMode.GRAPH -> hippoRag.ragQa(queries = listOf(example.question))
                                        HippoRagSemanticMode.DPR -> hippoRag.ragQaDpr(queries = listOf(example.question))
                                    }
                                val solution =
                                    checkNotNull(qaResult.solutions.firstOrNull()) {
                                        "HippoRag returned no solutions for ${example.id}"
                                    }
        ...
        println("Pipeline: HippoRag")
        println("Retrieval Mode: ${semanticMode.name.lowercase()}")
    ...
    private fun resolveInputPath(args: Array<String>): Path {
        ...
        if (args[0] == "--input") {
            ...
            if (value.isNullOrBlank()) {
                System.err.println("Usage: MusiQueHippoRagas [--input <path>] or MusiQueHippoRagas [<path>]")
                kotlin.system.exitProcess(1)
            }
