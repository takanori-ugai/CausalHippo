...
/**
 * Runs OpenAlex-introduction MusiQue evaluation using HybridRAG generation and RAGAS metrics.
 *
 * Metrics:
 * - Faithfulness
 * - Context precision
 ...
 * - Context relevance
 */
object OpenAlexIntroMusiQueRagas {
    ...
    fun main(args: Array<String>) {
        ...
        val resultsDir = Path.of("eval_results").resolve("openalex_intro_musique_ragas_$timestamp")
    ...
    private fun resolveInputPath(args: Array<String>): Path {
        ...
        if (args[0] == "--input") {
            ...
            if (value.isNullOrBlank()) {
                System.err.println("Usage: OpenAlexIntroMusiQueRagas [--input <path>] or OpenAlexIntroMusiQueRagas [<path>]")
                kotlin.system.exitProcess(1)
            }
    ...
    private fun writeTempConfig(
        ...
        val tempDir = Files.createTempDirectory("hybridrag-openalex-intro-ragas")
        tempDir.toFile().deleteOnExit()
        val configPath = tempDir.resolve("openalex-intro-ragas-config.json")
