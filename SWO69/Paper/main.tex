...
表\ref{tab:spearman_corr}に、全実行条件（$N=286$サンプル）から得られた各KG品質指標と検索・生成性能指標とのSpearman順位相関係数（$r_s$）および有意確率を示す。
...
さらに、システム間およびデータセット間の固定効果を統制した上で、KG品質次元が最終回答F1に与える影響を多変量OLS回帰分析（Fixed-effects OLS Regression）により検証した。表\ref{tab:regression_results}に回帰分析の結果（$N=195, R^2 = 0.386$）を示す。
...
回帰分析および相関分析の結果から、以下の重要な実証的知見が得られた：
\begin{enumerate}
    \item \textbf{実体網羅性（ER）の支配的正効果}：固定効果回帰モデルにおいて、Entity Recall（ER）の標準化回帰係数は $\beta = +1.080$ ($p < 0.001$) と正の要因の中で最大であり、他の構造・システム効果を統制した状況下でも、KG内のエンティティ想起率を高めることが最終回答F1を直接的かつ強力に押し上げる主因であることが実証された。
    \item \textbf{グラフ密度の非有意性（量と質の乖離）}：Graph Densityの回帰係数は $\beta = +0.282$ であるものの $p = 0.313$ と統計的に有意ではなく、単にエッジ数を増やしてグラフ密度を高めるだけでは回答品質の向上に直結しないことが確認された（仮説H2の支持）。
    \item \textbf{過剰エッジとノイズの影響}：密度の高いグラフでは、無関係な周辺ノードへの探索拡散が生じるため、RH@5やCiCとの単変量相関が負に振れる傾向がみられた。これより、Graph-Based RAGでは「全結合的な密グラフ」よりも「正解実体（ER）を確実に含みつつ不要なエッジを抑えた高精度な疎グラフ」が優位であることが示唆される。
\end{enumerate}

\subsection{抽出モデル能力階層操作（E1）}
生成LLM（\texttt{Qwen-2.5-27B}）および埋め込みモデルを固定したまま、KG構築を担う抽出LLMの能力を強モデル（\texttt{Qwen-2.5-27B}）、中モデル（\texttt{Gemma-2-9B} / \texttt{Gemma-4-E4B}）、弱モデル（\texttt{Llama-3.2-3B}）の3階層に操作した場合の性能推移を測定した。表\ref{tab:e1_extraction_results}に全5系統および3つの評価データセット（MuSiQue, Causal-Reasoning-QA, Webis-CausalQA-22）における品質・性能測定結果を示す。
...
表\ref{tab:e1_extraction_results}に示すように、評価データセットの種別（多段推論・因果推論・WebオープンQA）および手法（全5系統）を問わず、抽出LLMの能力低下に伴って実体想起率（ER）や関係想起率（RR）が大幅に減衰し、構文崩壊による抽出失敗率（EFR）が増加する。このKG品質の劣化が連鎖して検索レイヤにおける推論連鎖網羅率（CiC）を低下させ、最終回答生成のToken F1が全手法・全データセットで一貫して30〜38\%程度低下することが確認された。この結果は、データセット固有の特性や個別アルゴリズムの差異を超えて、KG構築段階の情報抽出能力がGraph-Based RAG全体の品質上限を普遍的に決定づけていることを実証している。
...
\begin{enumerate}
    \item \textbf{抽出フェーズにおける「想起性（Recall）」の最優先}：回帰分析で実証された通り、最終回答品質を最も強く押し上げるのはEntity Recall（$\beta = +1.080$）である。ノイズに対する頑健性（P5）が比較的高いため、抽出プロンプトでは過度なフィルタリングを避け、多様な同義語や関連概念を漏れなく拾い上げる設計が有効である。
