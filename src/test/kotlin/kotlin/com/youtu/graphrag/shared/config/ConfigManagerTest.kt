...
class ConfigManagerTest {
    ...
    fun `saveConfig persists json and yaml and round-trips config map`() {
        ...
        config.saveConfig(yamlPath.toString())
        val savedYamlText = yamlPath.readText()
        assertTrue(savedYamlText.contains("datasets:"))
        val reloadedYaml = ConfigManager(yamlPath.toString())
        assertEquals(config.toMap(), reloadedYaml.toMap())
    }
