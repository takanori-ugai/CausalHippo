...
usage() {
  cat <<'EOF_USAGE'
Usage: ./run_experiment_causal_graph.sh [options]
...
if [[ -z "$OUTPUT_DIR" ]]; then
  OUTPUT_DIR="eval_results/causal_graph_multicondition_$(date -u +%Y%m%d_%H%M%S)"
fi
...
echo "[run_experiment_causal_graph] output: $OUTPUT_DIR"
echo "[run_experiment_causal_graph] conditions: $CONDITIONS"
echo "[run_experiment_causal_graph] QA metrics: exact_match, precision, recall, f1"
echo "[run_experiment_causal_graph] BertScore metrics: bertscore_precision, bertscore_recall, bertscore_f1"
...
echo "[run_experiment_causal_graph] done"
echo "[run_experiment_causal_graph] per-question JSONL: $OUTPUT_DIR/per_question"
echo "[run_experiment_causal_graph] summary CSV: $OUTPUT_DIR/summary_by_condition.csv"
echo "[run_experiment_causal_graph] summary by category CSV: $OUTPUT_DIR/summary_by_condition_category.csv"
echo "[run_experiment_causal_graph] label-aware summary CSV: $OUTPUT_DIR/summary_label_aware_by_condition.csv"
echo "[run_experiment_causal_graph] per-question CSV: $OUTPUT_DIR/per_question_metrics.csv"
echo "[run_experiment_causal_graph] note: CSV outputs include BertScore columns"
